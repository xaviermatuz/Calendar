...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function hideurl_timeout_interval() {
    ...
    if (class_div_container.classList.contains("fc-daygrid")) {
      ...
      var listanchorclass = document.getElementsByClassName(
        "fc-event-forced-url"
      );
      setTimeout(function () {
  ...
  var calendarEl = document.getElementById("calendar"); //Invoke the Calendar in a variable from the view
  var calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    eventDrop: function (arg) {
      ...
      var start =
        arg.event.start.toDateString() +
        " " +
        arg.event.start.getHours() +
        ":" +
        arg.event.start.getMinutes() +
        ":" +
        arg.event.start.getSeconds(); // We get from the view the values of start time
      if (arg.event.end == null) {
        // If we didn't found any value for end set the start ones
        end = start; //If the end date is null we set the same start date to avoid exceptions
      } else {
        //Set the value and format of End date if provided
        var end =
          arg.event.end.toDateString() +
          " " +
          arg.event.end.getHours() +
          ":" +
          arg.event.end.getMinutes() +
          ":" +
          arg.event.end.getSeconds(); // We get from the view the values of end time
      }
      $.ajax({
    ...
    select: function (arg) {
      ...
      const prehou = today.getHours() < 10 ? "0" : ""; //Basic Validation of the Hours Lenght Content
      const premin = today.getMinutes() < 10 ? "0" : ""; //Basic Validation of the Minutes Lenght Content
      const presec = today.getSeconds() < 10 ? "0" : ""; //Basic Validation of the Seconds Lenght Content
      var time =
        prehou +
        today.getHours() +
        ":" +
      ...
      if (eR_DateLocal.test(arg.startStr) && eR_DateLocal.test(arg.endStr)) {
        //Test case for RegEx local ISO8086 String Week/Day
        const split_time_local_startStr = arg.startStr.split("T")[1]; //We save the Second part of the string in Start "HH:MM:SS-GMT"
        const split_time_local_endStr = arg.endStr.split("T")[1]; //We save the Second part of the string in End "HH:MM:SS-GMT"
        var dts =
          arg.startStr.split("T")[0] +
          " " +
          split_time_local_startStr.split("-", 1); //We assign to a variable the first string in start, "YYYY-MM-DD" then we add the second part but we split the "GMT"
        var dte =
          arg.endStr.split("T")[0] +
          " " +
          split_time_local_endStr.split("-", 1); //We assign to a variable the first string in end, "YYYY-MM-DD" then we add the second part but we split the "GMT"
      } else if (eR_DateUTC.test(arg.startStr) && eR_DateUTC.test(arg.endStr)) {
        //Test case for RegEx UTC ISO8086 String Week/Day
        const split_time_UTC_startStr = arg.startStr.split("T")[1]; //We save the Second part of the string in Start "HH:MM:SSZ"
        const split_time_UTC_endStr = arg.endStr.split("T")[1]; //We save the Second part of the string in End "HH:MM:SSZ"
        var dts =
          arg.startStr.split("T")[0] +
          " " +
          split_time_UTC_startStr.split("Z", 1); //We assign to a variable the first string in start, "YYYY-MM-DD" then we add the second part but we split the "Z"
        var dte =
          arg.endStr.split("T")[0] + " " + split_time_UTC_endStr.split("Z", 1); //We assign to a variable the first string in start, "YYYY-MM-DD" then we add the second part but we split the "Z"
      } else {
        //Test case for ISO8086 String UTC/Local MONTH
    ...
    $.ajax({
      ...
      if (data.success) {
        ...
        if (data.errors.start) {
          ...
          $("#start-date-group").append(
            '<div class="help-block">' + data.errors.start + "</div>"
          ); // Insertion of the message error for date to the view
        }
        if (data.errors.end) {
          // If error exist update the html
          $("#end-date-group").addClass("has-error"); // Insert the error class
          $("#end-date-group").append(
            '<div class="help-block">' + data.errors.end + "</div>"
          ); // Insertion of the message error for date to the view
        }
        if (data.errors.title) {
          ...
          ); // Insertion of the message error for title to the view
        }
        hideurl_timeout_interval(); //Call to Timeout function to Hide URL's
      }
      //
